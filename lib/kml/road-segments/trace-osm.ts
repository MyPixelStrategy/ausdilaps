// Free OSM-based tracer — the fallback used by trace.ts when GOOGLE_MAPS_API_KEY isn't
// configured. Google (trace-google.ts) is more reliable (handles divided roads and
// roads missing from OSM correctly — see its comments) but costs a paid API call; this
// path keeps the tool working for free without any billing setup.
//
...
export async function traceRoadSegmentsOsm(segments: RoadSegmentInput[]): Promise<RoadTraceResult[]> {
  ...
  await mapPool(Array.from(groups.entries()), 2, async ([key, rows]) => {
    ...
    try {
      waysByLocation.set(key, await fetchRoadsByNames(names, bboxAround(anchor, 6)));
    } catch {
